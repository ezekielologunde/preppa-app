...
/**
 * The meal browse experience (search + macro/goal/price filters + grid), reused by the
 * Explore route and the Discover "Meals" mode. Provides its own search + filters; the host
 * supplies the page/mode header.
 */
export function MealsBrowser({ initialCat, initialGoal }: { initialCat?: string; initialGoal?: string }) {
  const c = useC();
    ...
    <>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={{ paddingHorizontal: 16, paddingTop: 16, paddingBottom: 10 }}>
          <Text style={[type(20, 900), { color: c.ink, letterSpacing: -0.7 }]}>{loading ? 'Loading…' : `${list.length} meal${list.length !== 1 ? 's' : ''}`}</Text>
        </View>
      ...
      <Sheet visible={filterOpen} onClose={() => setFilterOpen(false)}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} style={{ flexShrink: 1 }} contentContainerStyle={{ paddingBottom: 10 }}>
          ...
          <FSec title="Price">{PRICES.map((p) => <Chip key={p.label} label={p.label} on={price === p.label} onPress={() => setPrice(price === p.label ? null : p.label)} />)}</FSec>
          <FSec title="Tags">{ALL_TAGS.map((t) => <Chip key={t} label={t} on={tags.includes(t)} onPress={() => toggleTag(t)} />)}</FSec>
        </ScrollView>
        <View style={{ paddingTop: 8 }}>
          <Btn label={`Show ${list.length} meal${list.length !== 1 ? 's' : ''}`} block onPress={() => setFilterOpen(false)} />
        </View>
      </Sheet>
    </>
  );
