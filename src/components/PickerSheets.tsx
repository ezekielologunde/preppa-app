...
/** Quick payment-method picker (bottom sheet) — stays in checkout context. */
export function CardPickerSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  const c = useC();
  const router = useRouter();
  const { cards, cardId, selectCard, toast } = useStore();
  const pick = (id: string) => { selectCard(id); toast('Payment method updated', 'card', true); onClose(); };
  return (
    <Sheet visible={visible} onClose={onClose} title="Payment method" scroll>
      {cards.length === 0 ? (
        <Text style={[type(14, 500), { color: c.soft, paddingHorizontal: 4, paddingVertical: 10 }]}>No saved cards yet.</Text>
      ) : (
        cards.map((cd) => {
          const on = cd.id === cardId;
          return (
            <Press key={cd.id} scale={0.99} onPress={() => pick(cd.id)} label={`Use ${cd.brand} ending ${cd.last4}`}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderRadius: radius.md, backgroundColor: on ? c.primaryL : 'transparent' }}>
                <Icon name="card" size={18} color={on ? c.primary : c.soft} />
                <Text style={[type(14.5, 800), { color: c.ink, flex: 1 }]}>{cd.brand} •••• {cd.last4}</Text>
                {on ? <Icon name="check" size={18} color={c.primary} /> : null}
              </View>
            </Press>
          );
      ...
      <View style={{ marginTop: 8 }}>
        <Btn label="Manage cards" icon="plus" variant="ghost" block onPress={() => { onClose(); router.push('/payments'); }} />
      </View>
    </Sheet>
  );
}
