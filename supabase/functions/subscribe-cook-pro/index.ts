...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existing } = await db.from('cook_memberships')
      .select('status, current_period_end, trial_consumed, stripe_subscription_id').eq('kitchen_id', kitchenId).maybeSingle();
    ...
    try {
      ...
      }, { idempotencyKey: `cookpro_sub_${kitchenId}_${interval}_${existing?.stripe_subscription_id ?? 'first'}` });
    } catch (e: any) {
      if (isAmbiguousStripeError(e)) {
        return json(503, { error: 'Stripe could not confirm the membership request. Retry safely; the kitchen will not be subscribed twice.', code: 'retryable_provider_error' });
      }
      return json(402, { error: e?.message || 'Your card was declined.', code: 'charge_failed' });
    }

    const startedTrial = sub.status === 'trialing';
    const { error: membershipError } = await db.from('cook_memberships').upsert({
