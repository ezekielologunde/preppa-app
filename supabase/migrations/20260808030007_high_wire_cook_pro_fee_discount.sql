-- Preppa Pro benefit #1: reduced processing-fee share. Non-member cooks bear the real Stripe
-- cost (~2.9% + 30c) per the comment on the original ledger 'fee' entries; Pro members pay a
-- flat 1% instead (no fixed 30c), Preppa absorbs the difference as a membership-funded perk.
-- Scoped to the 3 branches where a fee is charged against exactly ONE kitchen (direct orders,
-- plain subscription-cycle orders, bookings). Deliberately NOT applied to the cross-kitchen
-- box-split branch: that branch computes one blended stripe_fee for the whole box PI and
-- apportions it pro-rata by revenue share across however many kitchens are in the box: giving
-- one kitchen a discounted rate there would require restructuring that apportionment (not a
-- one-line change) and risks the trickiest part of this trigger. Left on the standard rate
-- for now -- worth a dedicated follow-up if box orders become a meaningful revenue share for
-- Pro members.
  ...
  begin
    if (new.metadata->>'order_id') ~ '^[0-9a-fA-F-]{36}$' then
      ...
      if found and not exists (select 1 from public.ledger_entries where order_id=o.id and kind='sale') then
        ...
        v_stripe_fee := case when public.is_cook_pro_member(o.kitchen_id)
                          then round(o.total_cents*0.01)::int
                          else round(o.total_cents*0.029)::int + 30 end;
      ...
      if found then
        ...
        if not exists (select 1 from public.ledger_entries where booking_id=b.id and kind='sale') then
          ...
          v_stripe_fee := case when public.is_cook_pro_member(b.kitchen_id)
                            then round(b.deposit_cents*0.01)::int
                            else round(b.deposit_cents*0.029)::int + 30 end;
          insert into public.ledger_entries(kitchen_id,booking_id,kind,amount_cents,memo)
