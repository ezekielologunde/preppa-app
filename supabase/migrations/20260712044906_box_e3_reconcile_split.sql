-- reconcile_paid_pi: add the BOX branch (Model B split-ledger). One settled PI → one
-- box_orders parent + N per-kitchen child orders + N sale/fee ledger rows (each cook credited
-- their full menu price; the single Stripe fee apportioned by largest-remainder so the cents
-- reconcile exactly). Idempotent (advisory lock + ON CONFLICT DO NOTHING), append-only-safe.
-- The single-kitchen cycle branch + order/booking branches are unchanged.
  ...
  begin
    if (new.metadata->>'order_id') ~ '^[0-9a-fA-F-]{36}$' then
      ...
      if not found then
        ...
        -- ---- BOX: split one charge across N cooks ----
        ...
        if cyc.box_order_id is null then
          ...
          loop
            insert into public.orders(customer_id, kitchen_id, box_order_id, status, method, pay_status, fulfillment,
              ...
              on conflict (idempotency_key) do nothing returning id into v_child;
        ...
        -- ---- single-kitchen cycle (unchanged) ----
