drop function if exists claim_cycles_for_charge(int);
create function claim_cycles_for_charge(p_limit int default 50)
...
begin
  ...
  update subscription_cycles set payment_status='charging', charge_attempts=charge_attempts+1, updated_at=now()
   where id = any(v_ids);
  return query
