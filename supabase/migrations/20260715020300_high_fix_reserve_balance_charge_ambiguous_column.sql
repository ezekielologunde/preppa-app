-- Caught by the live fixture test (booking 667527c9-...), not by CREATE FUNCTION validation:
-- reserve_balance_charge's re-read-under-lock query selected bare `balance_cents`, which is
-- ambiguous between the bookings.balance_cents table column and the function's own
-- RETURNS TABLE(..., balance_cents int, ...) out-parameter of the same name -- PL/pgSQL exposes
-- out parameters as assignable variables in scope, and check_function_bodies validation at
-- CREATE time doesn't catch this class of ambiguity (only failed at actual execution: "column
-- reference balance_cents is ambiguous"). Qualifying with the table name resolves it -- same
-- fix applied to the first SELECT already (it used the `b.` alias and never hit this).
...
begin
  ...
  -- (qualified with the table name: bare balance_cents collides with this function's own
