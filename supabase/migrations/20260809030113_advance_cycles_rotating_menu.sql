...
declare
  ...
  v_member boolean; v_week_idx int;
begin
  ...
  loop
    ...
    if v_res is null then
      ...
      if pl.selection_model='fixed' then
        -- rotating menu: pick this cycle's week slot from how many cadence-weeks have
        -- elapsed since the subscription's billing anchor, wrapped to the plan's rotation
        -- length; a plan with rotation_weeks=1 (the default / non-rotating) always uses
        -- week_index 0, i.e. exactly today's behavior.
        v_week_idx := 0;
        ...
        insert into subscription_cycle_items(cycle_id, meal_id, qty, kitchen_id, unit_price_cents)
          ...
          on conflict (cycle_id, meal_id) do nothing;
        -- fall back to week 0 if this plan has no items tagged for the computed week
        -- (e.g. rotation_weeks was raised without filling every week yet)
