-- Prevent an ambiguous subscription charge from being retried with a fresh idempotency key.
-- Only pre-Stripe crashes may be returned to pending by advance_cycles().
...
begin
  ...
  update subscription_cycles
   ...
   where payment_status='charging' and stripe_payment_intent_id is null
     and last_payment_error is distinct from 'ambiguous_stripe_outcome'
...
begin
  update public.subscription_cycles
     set last_payment_error = 'ambiguous_stripe_outcome', updated_at = now()
   where id = p_cycle
     and payment_status = 'charging'
