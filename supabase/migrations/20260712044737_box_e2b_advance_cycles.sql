-- A box cycle has no single kitchen (items carry their own kitchen_id).
alter table subscription_cycles alter column kitchen_id drop not null;
...
begin
  ...
  loop
    ...
    if s.kind = 'box' then
      ...
      -- reserve capacity per kitchen; drop any full kitchen's items (drop-and-reprice)
      for v_k in select distinct kitchen_id from subscription_cycle_items where cycle_id=v_cycle loop
        ...
        if reserve_capacity(v_k, v_delivery, v_portions, 'cycle', v_cycle) is null then
    ...
    if v_res is null then
      ...
      if pl.selection_model='fixed' then
        insert into subscription_cycle_items(cycle_id, meal_id, qty, kitchen_id, unit_price_cents)
          select v_cycle, pi.meal_id, pi.qty, s.kitchen_id, m.price_cents from plan_items pi join meals m on m.id=pi.meal_id where pi.plan_id=pl.id
          on conflict (cycle_id, meal_id) do nothing;
      end if;
      insert into subscription_events(subscription_id,cycle_id,event,to_status,actor)
        values(s.id, v_cycle, 'cycle_opened', 'selection_open', 'cron');
  ...
  loop
    ...
    if c.is_box then
      ...
      v_fee   := round(v_subtotal * coalesce(s.service_fee_bps,1500) / 10000.0)::int;
      v_total := v_subtotal - v_disc + v_fee;
      update subscription_cycles
         set subtotal_cents=v_subtotal, discount_cents=v_disc, service_fee_cents=v_fee, tax_cents=0,
             total_cents=v_total, status='selection_closed', updated_at=now()
       where id=c.id;
      insert into subscription_events(subscription_id,cycle_id,event,from_status,to_status,actor,meta)
        values(s.id, c.id, 'selection_closed', 'selection_open', 'selection_closed', 'cron',
               jsonb_build_object('total_cents', v_total, 'discount_cents', v_disc));
      continue;
    end if;
    ...
    if not exists (select 1 from subscription_cycle_items where cycle_id=c.id) then
      insert into subscription_cycle_items(cycle_id, meal_id, qty, kitchen_id, unit_price_cents)
        select c.id, pi.meal_id, pi.qty, s.kitchen_id, m.price_cents from plan_items pi join meals m on m.id=pi.meal_id where pi.plan_id=pl.id
        on conflict (cycle_id, meal_id) do nothing;
    end if;
    ...
    update subscription_cycles
     ...
     where id=c.id;
    insert into subscription_events(subscription_id,cycle_id,event,from_status,to_status,actor,meta)
      values(s.id, c.id, 'selection_closed', 'selection_open', 'selection_closed', 'cron', jsonb_build_object('total_cents', v_total));
  end loop;
