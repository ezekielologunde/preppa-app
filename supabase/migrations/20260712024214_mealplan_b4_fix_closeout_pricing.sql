-- Correct cycle pricing at closeout:
--   fixed          -> the cook's bundle price (plans.price_cents), not the a-la-carte meal sum
--   customer_choice -> per_meal_cents x selected qty, or the meal-price sum if per_meal_cents unset
-- (+ per_delivery_cents, service fee, tax). Trial overrides to trial_price_cents.
...
declare
  ...
  v_subtotal int; v_fee int; v_tax int; v_total int; v_qty int;
begin
