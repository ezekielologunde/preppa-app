-- Reuse a customer's active booking after a lost payment response instead of creating a second charge.
...
begin
  ...
  if e.price_model = 'flat' then
    ...
    v_amount := e.price_cents; v_hold := s.capacity;
  else
    if e.per_person_cents is null then raise exception 'experience missing price'; end if;
    v_amount := e.per_person_cents * p_guests; v_hold := p_guests;
  end if;
  v_fee := round(v_amount * coalesce(e.service_fee_bps, 1500) / 10000.0)::int;
  select * into v_existing from bookings
   where customer_id = v_cust and session_id = p_session
     and status in ('pending_deposit','confirmed','in_progress','completed')
   order by created_at desc limit 1;
  if v_existing.id is not null then
    -- The first attempt's amount and seat hold are immutable. Its Stripe idempotency key is
    -- booking-scoped, so changing guests here would make the saved booking disagree with the
    -- PaymentIntent that a retry must recover.
    return jsonb_build_object('bookingId',v_existing.id,'amountCents',v_existing.amount_cents,'deduped',true,
      'alreadyPaid',v_existing.status in ('confirmed','in_progress','completed'));
  end if;
  ...
  insert into bookings(kitchen_id,customer_id,booking_kind,experience_id,session_id,guests,amount_cents,deposit_cents,service_fee_cents,event_date,status,idempotency_key)
    values(e.kitchen_id,v_cust,'experience',p_experience,p_session,p_guests,v_amount,v_amount,v_fee,s.starts_at::date,'pending_deposit','exp_'||replace(gen_random_uuid()::text,'-',''))
    returning id into v_bid;
  insert into experience_seat_reservations(session_id,booking_id,guests) values(p_session,v_bid,v_hold);
  return jsonb_build_object('bookingId',v_bid,'amountCents',v_amount,'alreadyPaid',false);
end $body$;
