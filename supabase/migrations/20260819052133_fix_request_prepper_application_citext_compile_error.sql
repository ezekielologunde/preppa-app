-- The function declares "v_slug citext" but runs with SET search_path = 'public',
-- while the citext extension lives in the "extensions" schema. Every invocation
-- therefore failed at PL/pgSQL compile time with 42704 'type "citext" does not exist',
-- breaking the prepper application flow. Declaring v_slug as text fixes compilation;
-- text implicitly casts to the citext kitchens.slug column, so behavior is unchanged.
CREATE OR REPLACE FUNCTION public.request_prepper_application(
  ...
  p_bio text DEFAULT NULL::text, p_phone text DEFAULT NULL::text, p_address text DEFAULT NULL::text,
  p_food_safety jsonb DEFAULT NULL::jsonb, p_food_handler_cert text DEFAULT NULL::text,
  p_agreement_version text DEFAULT NULL::text, p_service_types text[] DEFAULT ARRAY['meals'::text],
  p_service_area text DEFAULT NULL::text, p_experience text DEFAULT NULL::text,
  p_verified_lat numeric DEFAULT NULL::numeric, p_verified_lng numeric DEFAULT NULL::numeric)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
