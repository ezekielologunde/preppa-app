-- Launch-prep gap: reconcile_paid_pi() settles payment (marks orders paid, writes ledger
-- entries) but never told the COOK a new order landed -- update_order_status() notifies the
-- CUSTOMER on every status change (confirmed->preparing->ready->completed), but nothing
-- notified the cook of the initial order itself. The realtime `notifications:${userId}`
-- channel + notifications table already exist and work (that's what powers the customer-side
-- updates) -- this was purely a missing notify() call at order-creation time, not missing
-- infrastructure. Today a cook only learns of a paid order by manually opening Orders and
-- pulling to refresh (useFocusEffect, not push).
--
-- Adds one `perform public.notify(...)` at each point in this trigger where a real, newly-
-- paid order/booking is created for a kitchen: the direct single-kitchen order path, each
-- child order in a split cross-kitchen box, the subscription-cycle order path, and a booking
-- deposit. notify() already no-ops on a null user and swallows its own exceptions, so this
-- cannot regress the payment/ledger logic it's appended to. Everything else in this function
-- is copied verbatim from the live definition -- no other behavior change.
