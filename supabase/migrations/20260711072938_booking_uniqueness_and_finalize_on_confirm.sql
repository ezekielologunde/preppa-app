-- At most one live booking per request (a second accept on the same request fails cleanly).
create unique index if not exists one_live_booking_per_request
  on public.bookings(request_id) where status not in ('cancelled','refunded','no_show');

-- On a confirmed booking, finalize the marketplace state: accept the winning quote, decline the
-- rest, close the request. Done in the reconcile trigger so acceptance is finalized only once the
-- deposit is actually paid.
...
declare
  ...
  o public.orders%rowtype; b public.bookings%rowtype; v_stripe_fee int;
begin
  ...
  begin
    if (new.metadata->>'order_id') ~ '^[0-9a-fA-F-]{36}$' then
      ...
      if found then
        -- finalize marketplace state (idempotent)
