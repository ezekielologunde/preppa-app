-- PrepPlus P4 (plans/boxes): waive Preppa's platform fee for members, LOCKED at cycle-snapshot
-- time (advance_cycles closeout) so the previewed total == charged total. Adds a fee_waived audit
-- flag. Only the two fee-source lines + the two closeout UPDATEs change vs the live function.
alter table public.subscription_cycles add column if not exists fee_waived boolean not null default false;
