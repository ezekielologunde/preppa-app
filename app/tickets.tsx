...
function TicketThread({ ticket, onReplied }: { ticket: MyTicket; onReplied: () => void }) {
  ...
  return (
    <View style={{ marginTop: 12, borderTopWidth: 1, borderTopColor: c.border2, paddingTop: 12, gap: 10 }}>
      ...
      {!error && ticket.status !== 'closed' ? <><TextInput value={reply} onChangeText={setReply} placeholder="Reply to support…" placeholderTextColor={c.muted} multiline accessibilityLabel="Reply to support" style={{ minHeight: 56, borderWidth: 1, borderColor: c.border, borderRadius: radius.md, padding: 12, color: c.ink, backgroundColor: c.bg2, textAlignVertical: 'top', ...(type(14, 600) as object) }} /><View style={{ alignItems: 'flex-end' }}><Btn label="Send reply" icon="arrow" loading={busy} disabled={!reply.trim()} onPress={send} height={44} /></View></> : null}
