...
- [~] Reduce reliance on `src/data/data.ts` mock data; audit which persisted-store seed data is still reachable. Progress 2026-09-25: removed the dormant mock catalog repository, dead meal and notification fixtures, the seed-slug daily-drop selector, seed-only founding-cook fee presentation, and the obsolete device-local subscription model. Home now rotates its featured item through the live catalog, checkout preview mirrors server fee rules, real catalog search uses server kitchen names only, and malformed historical order lines fall back to a neutral kitchen label instead of a named seed cook. Explicit prototype experiences and their presentation metadata remain for a later product decision.
