...
**Unresolved from direct inspection** (didn't render conclusively during the walkthrough, don't treat as confirmed either way): whether Shef's Gift Cards feature is actually fully live (the footer link resolved to a blank/redirected page both times); the exact checkout-time fee breakdown (delivery fee/service fee/tax split) — cart never got past an empty state during this pass.
