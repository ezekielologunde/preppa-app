...
- [ ] **Fix Preppa's Auth SMTP sender**: discovered while wiring the above — Supabase Auth's SMTP config uses a Resend key from an unrelated "getbagsly" workspace (`mail.bagsly.co` domain), not Preppa's own verified `preppa.live` domain. OTP/signup emails currently show that domain as sender. Should be repointed at the Preppa Resend account (already has a verified `preppa.live` domain — same one now used for admin alerts).
