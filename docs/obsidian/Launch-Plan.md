...
- [x] ~~Livestreaming — already flag-disabled~~ — confirmed again 2026-09-25: `FLAGS.live = false`; the viewer, feed, storefront, and deep-linked go-live routes gate on it, the cook hub now removes its go-live shortcut while disabled, and a separate server-side `LIVE_ENABLED` kill switch remains independent of the client flag.
