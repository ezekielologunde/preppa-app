...
- [x] ~~System health~~ — **partially done 2026-09-08**, the DB-observable half. `detect_system_health_issues()` (pg_cron, every 15 min) alerts via `notify_admins()` on (1) `cron.job_run_details` showing a scheduled job's SQL statement actually failed, and (2) `net._http_response` showing a real outbound HTTP failure (5xx or timeout) from any `pg_net` call — covers reconcile-payouts, auto-payouts, detect-admin-anomalies, notify_admins' own dispatch, stripe-setup-nudge. Verified live: cron job registered and active, function runs clean (112/112 recent scheduled runs succeeded, correctly triggered no false alarm). **Not covered, deliberately**: Vercel deploy status and Stripe webhook delivery failures live entirely outside this database — both platforms already have their own native alerting for free (Vercel deployment notifications, Stripe Dashboard webhook failure emails); recommend turning those on directly rather than building a custom poller that would need a new external API token for comparatively little gain.
