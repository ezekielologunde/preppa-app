...
- [~] **No demo/seed data reachable as if real inventory — partially open, needs a decision.** Checked directly against the live DB: the 6 hardcoded seed cooks in `src/data/data.ts` (`maria`/`david`/`amara`/`denise`/`lucia`/`sana`) map via `KITCHEN_ID` (`src/lib/supabase.ts`) to 6 real rows in the production `kitchens` table, each still `verification_status = 'pending'` with 1-2 real `meals` rows attached (9 meals total) — literal placeholder/demo data sitting in the live database, not just client-side mock data. **Confirmed NOT currently customer-reachable**: RLS (`kitchens_select_verified_public`, `meals_select_live_public`) only exposes `verified` kitchens to anon/customer queries, so these 9 fake meals cannot appear in Discover today. The client-side fallback code that would render them richly (`seedCookForKitchen` in `supabaseRepository.ts`, consumed by 8 screens) is therefore currently dead code, not an active leak — but the fake rows themselves are still real production clutter and a landmine if verification status or RLS ever changes. **Needs a decision**: delete the 6 fake kitchens/9 meals from production and remove the `COOKS`/`KITCHEN_ID`/`seedCookForKitchen` fallback code, or confirm they're intentionally kept for some other reason (a demo account, screenshots, etc.) before touching them.
