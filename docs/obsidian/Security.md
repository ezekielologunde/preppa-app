...
- **Found in the process, not yet fixed:** Preppa's live Supabase Auth SMTP (the sender for OTP/signup emails) is configured with a Resend key from a *different*, unrelated Resend workspace ("getbagsly", domain `mail.bagsly.co`) rather than the Preppa-owned `preppa.live` domain now used above. Recipients see `mail.bagsly.co` as the sender today. See [[Tasks]].
